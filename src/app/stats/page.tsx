...
export default function StatsPage() {
  ...
  useEffect(() => {
    ...
    if (loadedPlayers.length > 0 && loadedMatches.length > 0) {
      const stats: PlayerStats[] = loadedPlayers.map((player: Player) => {
        ...
        const playerScores: number[] = loadedMatches.flatMap((match: Match) =>
          match.players.filter((p) => p.id === player.id).map((p) => p.score)
        );
        ...
        const averageScore: number =
          playerScores.length > 0 ? totalScore / playerScores.length : 0;
        const highestScore: number =
          playerScores.length > 0 ? Math.max(...playerScores) : 0;
  ...
  const bestPlayer: PlayerStats | null =
  ...
  const highestScoringPlayer: PlayerStats | null =
    playerStats.length > 0
      ? playerStats.reduce((prev, current) =>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-[#1e293b] rounded-2xl p-5 shadow-[inset_0_1px_1px_#ffffff10,_0_8px_20px_#0000003d] relative overflow-hidden group">
                ...
                <h3 className="text-[#a0aec0] text-sm mb-1">
                  Total des parties
                </h3>
                <p className="text-2xl font-bold text-[#e2e8f0] group-hover:scale-110 transform transition-transform origin-left duration-300">
                  {totalGamesPlayed}
                </p>
              </div>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-3 gap-3 text-sm">
                        ...
                        <div className="bg-[#1a253a] rounded-lg p-3 border border-[#303f60]/20">
                          ...
                          <p className="font-bold text-[#4facfe]">
